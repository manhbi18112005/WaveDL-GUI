...
from utils.schedulers import get_scheduler, list_schedulers, is_epoch_based
...
class TestListSchedulers:
    ...
    def test_contains_expected_schedulers(self):
        ...
        result = list_schedulers()
        expected = ['plateau', 'cosine', 'cosine_restarts', 'onecycle', 
...
class TestGetScheduler:
    ...
    @pytest.mark.parametrize("sched_name", ['plateau', 'cosine', 'cosine_restarts', 
                                            'step', 'multistep', 'exponential', 'linear_warmup'])
    ...
    def test_plateau_parameters(self, optimizer):
        ...
        scheduler = get_scheduler('plateau', optimizer, patience=5, factor=0.2, min_lr=1e-7)
...
class TestSchedulerStep:
    ...
    def test_linear_warmup_increases_lr(self, optimizer):
        ...
        scheduler = get_scheduler('linear_warmup', optimizer, warmup_epochs=5, start_factor=0.1)
