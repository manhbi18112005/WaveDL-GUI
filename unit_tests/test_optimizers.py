...
from utils.optimizers import get_optimizer, list_optimizers, get_optimizer_with_param_groups
...
class TestGetOptimizer:
    ...
    @pytest.mark.parametrize("opt_name", ['adamw', 'adam', 'sgd', 'nadam', 'radam', 'rmsprop'])
    ...
    def test_adamw_default(self, model):
        ...
        optimizer = get_optimizer('adamw', model.parameters(), lr=1e-3, weight_decay=1e-4)
    ...
    def test_sgd_with_nesterov(self, model):
        ...
        optimizer = get_optimizer('sgd', model.parameters(), lr=1e-2, momentum=0.9, nesterov=True)
...
class TestGetOptimizerWithParamGroups:
    ...
    def test_creates_param_groups(self, model):
        ...
        optimizer = get_optimizer_with_param_groups('adamw', model, lr=1e-3, weight_decay=1e-4)
